...
// map.go contains the functions to compose the SVG for the different types of maps
// as well as the path for the base map of Tasmania.
...
// drawGridMap draws a grid of 10 km x 10 km squares along the UTM MGA94 datum grid.
// It adds the grid identification numbers and draws the base outline map of Tasmania
func drawGridMap() {
...
// drawPlainMap only draws the path for the outline map of Tasmania
func drawPlainMap() {
	addCompactMap()
}

// makeGrid calculates separate grids for King Island and the rest of Tasmania, and
// draws grid lines every 10 km along the UTM MGA94 grid.
func makeGrid() {
...
// setNumbers writes the grid identification numbers for grid maps.
func setNumbers() {
...
// gridBox composes the information box for grid maps, containing the taxon name, number of grid
// cells occupied by observations, the total number of records, and the date.
...
// plainBox composes the information box for plain maps, containing the taxon name,
// the total number of records, and the date.
...
// webInfo composes the information for web maps used in the FTO, containing the taxon name,
// the total number of records, and the date.
func webInfo(r *RecordList) {
	recString := "Total records: " + strconv.Itoa(r.numRecs)
...
// addCompactMap draws the outline map of Tasmania.
func addCompactMap() {
